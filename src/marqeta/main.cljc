(ns marqeta.main
  "Kotodama WASM entrypoint for the Marqeta clean-room actor (L4) — Clojure port.
  ...
  facts (`marqeta.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "marqeta")
(def tier "L4")
...
(def entity-specs
  [{:entity "Card"          :plural "cards"          :id-prefix "marqeta_car"
    :fields [:token :state :lastFour :cardProductToken :userToken :fulfillmentStatus :createdTime]
    :required [:token :state]
    :coerce {}                       :refs {}}
   {:entity "CardProduct"   :plural "cardproducts"   :id-prefix "marqeta_car"
    :fields [:token :name :active :createdTime]      :required [:token :name]
    :coerce {:active :bool}          :refs {}}
   {:entity "User"          :plural "users"          :id-prefix "marqeta_use"
    :fields [:token :firstName :lastName :email :status :active :createdTime]
    :required [:token :firstName]
    :coerce {:active :bool}          :refs {}}
   {:entity "Transaction"   :plural "transactions"   :id-prefix "marqeta_tra"
    :fields [:token :amount :type :state :cardToken :userToken :createdTime]
    :required [:token :amount]
    :coerce {:amount :float}         :refs {}}
   {:entity "FundingSource" :plural "fundingsources" :id-prefix "marqeta_fun"
    :fields [:token :type :active :isDefaultAccount :createdTime]
    :required [:token :type]
    :coerce {:active :bool :isDefaultAccount :bool} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"marqeta.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "marqeta-compat" :tier tier :entities entities} 200])
...
(defn start! [] :marqeta-compat/ready)
